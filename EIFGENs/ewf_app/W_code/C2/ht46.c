...
extern void F46_641(EIF_REFERENCE);
...
void F46_641 (EIF_REFERENCE Current)
{
	GTCX
...
#define ur1 ur1x.it_r
	...
	RTEAA(l_feature_name, 45, Current, 0, 0, 708);
	RTSA(dtype);
	RTSC;
	RTME(dtype, 0);
	RTGC;
	RTDBGEAA(45, Current, 708);
	RTIV(Current, RTAL);
	RTHOOK(1);
	RTDBGAA(Current, dtype, 622, 0xF800012C, 0); /* vary_header_value */
	...
	*(EIF_REFERENCE *)(Current + RTWA(622, dtype)) = (EIF_REFERENCE) RTCCL(tr1);
	if (RTAL & CK_ENSURE) {
		...
		ur1 = RTCCL(tr1);
		tb1 = (((FUNCTION_CAST(EIF_TYPED_VALUE, (EIF_REFERENCE, EIF_TYPED_VALUE)) RTWF(626, dtype))(Current, ur1x)).it_b);
		if (tb1) {
			RTCK;
		} else {
			RTCF;
